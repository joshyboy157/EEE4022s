...
#next block
#display(plot(t1[39500:40000],y[39500:40000],xlabel = "Time (S)", ylabel = "Amplitude"))

# #next block

Y = fft(y)
n = length(Y)
f_axis = (0:n-1)*sample_rate/nsamples
Y1 = abs.(Y)

# #next block

#display(plot(f_axis[1:500000],Y1[1:500000],xlabel = "Frequency Hz",ylabel = "Magnitude"))
...
Sigma = [0.7, 1, 2, 3, 4, 5, 7,9 ,13, 15, 20 ,30 ,40,50,70,100,130,150,180,200,500,1000]
...
# display(plot(f_axis,abs.(lim_Noise1[1])))
...
#display(plot(t1[4000:5000],sent[1][4000:5000],xlabel = "Frequency (Hz)", ylabel = "Amplitude"))
...
#display(plot(f_axis[3000:500000],SENT[3000:500000],xlabel = "Frequency (Hz)",ylabel = "Magnitude"))
...
for i in 1:length(sent)
    ...
    for num in 1:its
        ...
        iq = sum(part .* exp.(2*π*f1*t*im))*Δt
        demod[num] = iq
        iq = sum(part .* exp.(2*π*f2*t*im))*Δt
        demod1[num] = iq
